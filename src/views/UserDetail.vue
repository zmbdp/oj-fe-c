          ...
          <div class="ku-user-section">
              ...
              <div class="photo-image-box">
                ...
                <div v-if="!isDisabled()">
                  <el-upload :action="uploadUrl" :headers="headers" :on-error="handleUploadError"
                    ...
                    <template #tip>
                      <span style="font-size: 12px; color: #ff0000;">*头像大小不超过5mb</span>
                    </template>
                  </el-upload>
                </div>
              </div>
            ...
            <el-form-item prop="phone">
              ...
              <label class="el-form-item__label">
                手机
                <span class="form-help-text">*输入错误则无法正常登录，请谨慎修改</span>
              </label>
              <!-- 输入框 -->
            ...
            <el-form-item label="个人介绍" prop="introduce">
              <el-input v-if="!isDisabled()" :rows="5" type="textarea" v-model="userDetailForm.introduce"
                autocomplete="off" placeholder="请用一段话介绍自己,该简介将会在您的个人主页展示" maxlength="80" show-word-limit></el-input>
              <span :class="userDetailForm.introduce ? 'right-info intro' : 'right-info intro gray'" v-else>
                {{ userDetailForm.introduce || "关于你的个性，兴趣或经验，展现自己独一档的强度..." }}
              </span>
            </el-form-item>
          </div>
...
async function editUser() {
  var res = await editUserService(userDetailForm)
  if (res.code === 1000) {
  ...
  .oj-user-info {
    ...
    &.oj-user-info-page {
      ...
      .user-info-edit {
        ...
        margin-left: auto;
        border: 2px solid #32C5FF; // 添加边框，颜色可以自定义
        padding: 6px 12px; // 添加内边距，使文本不贴边
        border-radius: 4px; // 边框圆角
        color: #32C5FF; // 文字颜色与边框颜色一致
        background-color: transparent; // 背景透明
      }

      .user-info-edit:hover {
        background-color: #f1faff; // 添加鼠标悬停时的背景色变化
